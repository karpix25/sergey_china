...
- Установи "is_product": false, если в ЛЮБОЙ части видео (даже на 1 секунду) появляется:
    * ПРЯМОЙ интерфейс TikTok (кнопки, профиль, наложенные поверх видео).
    * Чистый скриншот или запись экрана телефона (без рук, без физического устройства в кадре).
    * Призывы "link in bio", "click the link" во всплывающих элементах.
- Установи "is_product": true, ЕСЛИ:
    * Видео показывает РЕАЛЬНЫЙ физический товар.
    * Интерфейс (приложения, сайт) виден НА ЭКРАНЕ самого физического устройства, которое человек держит в руках или которое стоит на столе. Это ДОПУСТИМО.
    * Весь ролик посвящен демонстрации предмета или его работы в реальности.

2. ПАРАМЕТРЫ:
- "detected_duration": Длительность видео в секундах.
- "script": Текст озвучки на русском языке. 
    * ПРАВИЛО: ОБЯЗАТЕЛЬНО используй букву 'ё' (всё, ещё, шёл, жёлтый) — это критически важно для правильного ударения в ElevenLabs.
    * УДАРЕНИЯ: Если слово редкое, техническое или имеет двоякое ударение (зАмок/замОк, хлОпок/хлопОк), ВЫДЕЛЯЙ ударную гласную БОЛЬШОЙ буквой (например: "этот замОк очень надежный").
